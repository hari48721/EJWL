...
<form action="<?=site_url('Account_Master/get_data')?>" class="form-inline" method="POST">
<div class="modal-body">
Tax Type: <select  style="height: 28px; width:280px; " id="select" class="countries" name="tax" >
<option></option>
<?php foreach ($t as $row ): ?> 
<option value="<?=$row['taxtype']?>" <?php echo set_select('taxtype', $row['taxtype']); ?>><?=$row['taxtype']?></option> 
<?php endforeach ?>
</select>
</div>
<div class="modal-footer text-center">
<button type="submit" id="Find" class="btn btn-primary">Find<i class="icon-search4 position-right"></i></button></div>
</form>
</div>
</div></div>

<div class="content-wrapper">
<div id="modal_form_inline1" class="modal fade">
<div class="modal-dialog">
<div class="modal-content text-center">
<div class="modal-header">
<h5 class="modal-title">Tax</h5>
</div>
<form action="<?=site_url('Account_Master/Tax_Delete')?>" class="form-inline" method="POST">
<div class="modal-body">
Tax Type: <select  style="height: 28px; width:280px; " id="select" class="countries" name="taxtype" >
<option></option>
<?php foreach ($t as $row ): ?> 
<option value="<?=$row['taxtype']?>" <?php echo set_select('taxtype', $row['taxtype']); ?>><?=$row['taxtype']?></option> 
<?php endforeach ?>
...
<div class="modal-footer text-center">
<button type="submit" id="Delete" class="btn btn-primary">Delete<i class="icon-bin position-left"></i></button></div>
</form>
</div>
</div></div>
	
<form action="<?=site_url('Account_Master/Tax_Save')?>" method="POST">
<head><h5><center>TAX SLAB ENTRY</center></h5><button type="submit" id="button1" class="btn bg-slate"><i class="icon-file-plus position-left"></i>Ok</button>
<a  href="#" data-toggle="modal" id="button" data-target="#modal_form_inline"class="btn bg-info"><i class="icon-book3 position-left"></i> View</a>
<a  href="#" data-toggle="modal" id="button" data-target="#modal_form_inline1"class="btn bg-info"><i class="icon-bin position-left"></i>Delete</a>
...
<div id="item2">
Tax Percentage: <input type="text" style="height: 28px; width:100px; " id="taxper"  name="taxper">
</div>
...
<div id="item9">
CST Type:  <input type="text" style="height: 28px; width:50px; "  name="csttyp" id="csttyp"></div>
</form>
</body>
