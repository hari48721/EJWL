...
<form action="<?=site_url('Account_Master/Grp_Master_Update/'.$result->gid); ?>" method="POST">
<head><h5><center>Group Master</center></h5>
<button type="submit" class="btn bg-slate"><i class="icon-spinner9 position-left"></i> Update</button>
<a id="button" href="<?=site_url('Account_Master/Group_Master_View')?>" class="btn bg-info"><i class="icon-book3 position-left"></i> View All</a>
<a href="<?=site_url('Account_Master/Group_Master')?>" id="button" class="btn btn-info btn-xs"><i class="icon-cross2 position-left"></i>Cancel</a>
</head>
...
<div id="item">
Group Name:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;   <input type="text"style="height: 28px; width:350px; " name="grpname" autofocus required value="<?php echo $result->grpName; ?>">
</div>
...
<option value="<?=$row['gtid']?>"  <?php echo ($result->grptyp == $row['gtid']) ? 'selected="selected"' : '' ?> > 
<?=$row['grp_type']?></option>
<?php endforeach ?>
</select>
</div>
